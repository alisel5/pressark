...
class PressArk_Activity_Trace {
	...
	public static function reason_catalog(): array {
		return array(
			...
			'routing_decision_recorded' => 'The run recorded the chosen provider/model route and fallback assumptions.',
			...
			'preflight_blocked'       => 'Preflight blocked a tool call before execution.',
			'preflight_reroute'       => 'Preflight rerouted a tool call to the canonical family.',
			'preflight_rewrite'       => 'Preflight rewrote tool parameters before execution.',
			'hidden_tool_surface'     => 'Tool visibility removed hidden tools from the model-facing surface.',
			'requested_group_unreachable' => 'Requested tool groups resolved to no visible surface.',
			'entitlement_denied'      => 'Entitlements blocked the requested operation.',
			'discover_no_hits'        => 'Tool discovery returned no visible candidates for the query.',
			'discover_budget_reached' => 'Tool discovery hit its configured call budget.',
			'discover_repeated_misfire' => 'Repeated discovery attempts failed to find the right visible tool family.',
			...
			'queue_handoff'           => 'Foreground work was handed off to a background worker.',
			'worker_claimed'          => 'A background worker claimed the queued task.',
			'worker_deferred'         => 'The worker yielded and re-queued the task for a later attempt.',
			'worker_slot_contention'  => 'The worker could not acquire a concurrency slot and deferred.',
			'worker_completed'        => 'The background worker completed its task lifecycle.',
			'worker_cancelled'        => 'The background worker stopped because the parent run was cancelled.',
