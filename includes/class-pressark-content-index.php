 ...
 * --------------------------------------------------------------------------
 * SQL safety note:
 * Every $wpdb->prepare() call in this class uses literal SQL with %s/%d
 * placeholders. The only "interpolated" tokens are:
 *   - $this->table        → set in __construct() to $wpdb->prefix . 'pressark_content_index'
 *   - $wpdb->posts        → core wpdb table reference
 *   - $type_placeholders  → comma-separated list of %s tokens generated by array_fill()
 *   - $where              → composed of literal SQL fragments + %s/%d placeholders
 * No user data is ever interpolated into the SQL string itself; all values are
 * bound through prepare(). The lint rules below cannot statically prove this,
 * hence the class-scoped suppression. The two Placeholders.* rules below are
 * also disabled because the linter cannot count placeholders/args across
 * array_fill() interpolation or array_merge()+splat argument forwarding;
 * those are also runtime-correct.
 * --------------------------------------------------------------------------
 */
// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
...
// phpcs:enable WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
