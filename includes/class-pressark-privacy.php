...
class PressArk_Privacy {
	...
	public function add_privacy_policy_content(): void {
		...
		$content .= '<p>' . __( 'Chat messages, conversation history, system prompts, tool schemas, screen context, and content excerpts may be sent to the configured AI provider (OpenRouter, OpenAI, Anthropic, DeepSeek, or Google Gemini) to generate responses.', 'pressark' ) . '</p>';

		$content .= '<p>' . __( 'Freemius handles licensing, account and plan management, opt-in usage analytics, subscriptions, hosted checkout, credit purchases, and software update/account screens. Freemius may receive site URL/domain, WordPress/PHP/plugin version data, plugin slug, Freemius install ID, license/subscription identifiers, license keys/tokens, administrator name and email address supplied to Freemius, selected plan/pricing ID, billing cycle, license quantity, PressArk checkout-intent ID, payment ID and gross amount after checkout, and related purchase metadata. PressArk does not intentionally send site content to Freemius.', 'pressark' ) . '</p>';

		$content .= '<p>' . __( 'In bundled service mode, the PressArk Token Bank manages site registration, credit status, credit reservation, settlement, release, trace diagnostics, and AI proxy routing. It may receive site URL, site domain, installation UUID, site nonce, Freemius install ID when available, PressArk site token, correlation/reservation IDs, WordPress user ID, plan tier, selected AI provider/model, estimated and actual ICUs/tokens, input/output/cache token counts, and diagnostic trace metadata. For bundled AI proxy requests, it also receives and forwards the AI request body, which may include chat messages, conversation history, system prompts, tool schemas, and selected site/screen/content context.', 'pressark' ) . '</p>';

		$content .= '<p>' . __( 'In BYOK (Bring Your Own Key) mode, AI requests are sent directly to your chosen provider using the API key stored on your site. No bundled credits are consumed and no data is sent to PressArk services for AI billing. For OpenRouter BYOK requests, PressArk sends an HTTP referrer based on your site URL.', 'pressark' ) . '</p>';
		...
		$content .= '<p>' . __( 'If Telegram notifications are enabled for automations, your Telegram chat ID and bot token are stored encrypted. Notification messages are sent via the Telegram Bot API using the bot token in the request URL and may include the chat ID, message subject and body, optional wp-admin link, Markdown parse mode, web-preview setting, and summaries of actions performed.', 'pressark' ) . '</p>';
